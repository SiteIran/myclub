...
class AuthController extends Controller
{
    ...
    public function authEmail(Request $request) {
        ...
        if ($check) { // Check time for sending OTP for current mobile
            ...
            return response(['message' => 'به منظور ارسال مجدد کد به مدت 2 دقیقه منتظر بمانید.'], 201);
        }
        ...
        $found = PasswordReset::where('token', $request->token)->first();
            if ($found) { // Check token is valid
                ...
                if ($check) { // Check time expire, Discontinue
                    return response(['message' => 'زمان استفاده از کد تایید منقضی گردیده است'], 201);
                }
                ...
                return response(['message' => 'کد وارد شده صحیح نمی باشد'], 201);
            }
    }
